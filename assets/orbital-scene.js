...
const host = document.querySelector('.orbital-scene');
const section = document.querySelector('.visual-break');
...
if (host && section && !reduce.matches && !matchMedia('(prefers-reduced-data: reduce)').matches) {
  ...
  try {
    ...
    group.position.set(1.75, 0, 0);
    scene.add(group);
    const materials = [];
    for (const [radius, tilt, color, opacity] of [
      [1.7, 0.45, 0x4d80ff, 0.43],
      [1.38, -0.65, 0x00e0ff, 0.38],
      [1.05, 1.15, 0x7aa8ff, 0.31],
      [0.75, -1.1, 0x00e0ff, 0.24]
    ]) {
      const material = new THREE.MeshBasicMaterial({color, transparent:true, opacity, depthWrite:false});
      materials.push(material);
      const ring = new THREE.Mesh(new THREE.TorusGeometry(radius, 0.007, 4, 180), material);
    ...
    const resize = () => {
      ...
      camera.position.z = width < 700 ? 11.5 : 9;
      group.position.x = width < 700 ? 1.75 : 2.2;
    ...
    renderer.domElement.addEventListener('webglcontextlost', e => {
      ...
      host.classList.remove('is-ready'); host.dataset.renderer = 'fallback';
    });
  } catch (error) {
